...
//     const condTxs = await app.queryFilter(app.filters.NewConditionalTx());
//     const txObjAddr = condTxs[condTxs.length - 1].args.txObj;

//     const txObj = await ethers.getContractAt("ConditionalTx", txObjAddr);
//     await txObj.eval();
...
//     it.only("loadLocal uint256 NUMBER (1000) > loadLocal uint256 NUMBER2 (15)", async () => {
//       // Set NUMBER
...
//     it.only("loadRemote uint256 NUMBER (1000) > loadRemote uint256 NUMBER2 (15)", async () => {
//       // Set NUMBER
//       const bytes32Number = hex4Bytes("NUMBER");
//       await app.setStorageUint256(bytes32Number, 1000);
