  ...
  return [
    ...
    // Note: for now we're assuming that we have only one LP
    {
      ...
      conditions: [
        ...
        '(loadLocal uint256 LP_TOTAL) setUint256 LP_TOTAL_PREV',
        `(
    ...
    },
    {
      ...
      conditions: [
        ...
        `(uint256 2 * loadLocal uint256 LP_TOTAL / uint256 98 - loadLocal uint256 GP_INITIAL
    ...
    //   conditions: [
    //     '(loadLocal uint256 LP_TOTAL * uint256 2 / uint256 100) setUint256 MANAGEMENT_FEE',
    //     `(uint256 100 * loadLocal uint256 MANAGEMENT_FEE
    //        <= uint256 2 * loadLocal uint256 LP_TOTAL)`,
    //   ],
    // },
    // {
    //   txId: 72,
...
export const businessCaseTwoLPsSteps = (
  ...
  // Note: for now we're assuming that we have only one LP
  {
    txId: 2,
    requiredTxs: [1],
    signatories: [LP1.address, LP2.address],
    transaction: `(transferFromVar DAI LP TRANSACTIONS_CONT LP_INITIAL)
          and
