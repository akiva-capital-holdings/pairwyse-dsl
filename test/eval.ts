      ...
      describe("opLoadRemoteUint256", () => {
        it("17 > 15", async () => {
          ...
          await context.setProgram(
            `0x0b${number}${externalApp.address.substr(
              2,
            )}0b${number2}${externalApp.address.substr(2)}04`,
          );
        ...
        it("5 <= 3", async () => {
          ...
          await context.setProgram(
            `0x0b${number}${externalApp.address.substr(
              2,
            )}0b${number2}${externalApp.address.substr(2)}06`,
          );
          await externalApp.eval();
          await checkStack(StackValue, stack, 1, 0);
        });
        ...
        it("12 = 12", async () => {
          ...
          await context.setProgram(
            `0x0b${number}${externalApp.address.substr(
              2,
            )}0b${number2}${externalApp.address.substr(2)}01`,
          );
          await externalApp.eval();
          await checkStack(StackValue, stack, 1, 1);
        });
