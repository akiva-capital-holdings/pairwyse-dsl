...
initiating funds\x1b[0m
  ...
  before(async () => {
    ...
    await addSteps(preprocessorAddr, businessCaseSteps(GP, [LPs[0]], 4), agreementAddr);

    // `base = 5` - steps for businessCases with multiple LPs
    await addSteps(preprocessorAddr, businessCaseSteps(GP, [LPs[0], LPs[1]], 5), agreementAddr);
  ...
  describe('Lifecycle Test Multiple LPs', () => {
    const BASE = '5'; // base of transaction ID
    businessCaseTest(
  ...
  describe('Lifecycle Test one LP', () => {
    const BASE = '4'; // base of transaction ID
    businessCaseTest(
