...
import { Token, Agreement } from '../../typechain-types';
...
describe.only('Agreement: business case', () => {
  ...
  const businessCaseTest = (
    ...
    it(name, async () => {
      ...
      await agreement.setStorageAddress(hex4Bytes('TRANSACTIONS_CONT'), agreementAddr); // TODO: rename variable
      ...
      let result = false;
      try {
        const txn1 = await agreement.connect(GP).execute(41); // TODO: replance `41` with `base.concat(1)`
      ...
      if (!GP_FAILS_TO_DO_GAP_DEPOSIT) {
        ...
        await dai.connect(GP).approve(agreementAddr, GP_REMAINING);
      ...
      if (!GP_FAILS_TO_DO_GAP_DEPOSIT) {
        ...
        let DAI_BAL_OF_TXS = await dai.balanceOf(agreementAddr);
...
initiating funds\x1b[0m
        ...
        await dai.connect(whale).approve(agreementAddr, GP_PURCHASE_RETURN);
        ...
        DAI_BAL_OF_TXS = await dai.balanceOf(agreementAddr);
  ...
  // before(async () => {
  //   agreementAddr = process.env.AGREEMENT_ADDR || '';
  ...
  //   await agreement.setStorageUint256(hex4Bytes('LP_TOTAL'), 0);
  //   await agreement.setStorageUint256(hex4Bytes('GP_REMAINING'), 0);
  //   await agreement.setStorageUint256(hex4Bytes('TWO_PERCENT'), 0);
  ...
  before(async () => {
    ...
    await addSteps(preprocessorAddr, businessCaseSteps(GP, [LPs[0], LPs[1]], 4), agreementAddr);
