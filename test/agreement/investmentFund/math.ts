...
describe('Agreement: Investment Fund tests math', () => {
  ...
  const businessCaseTestDivisionErrors = (
    ...
    it(name, async () => {
      ...
      await agreement.setStorageUint256(hex4Bytes('PLACEMENT_DATE'), BigNumber.from(NEXT_MONTH));
      ...
      for await (const [i, LP_INITIAL] of LP_INITIAL_ARR.entries()) {
        ...
        await agreement.setStorageUint256(
          hex4Bytes('CLOSING_DATE'),
          BigNumber.from(NEXT_TWO_MONTH)
        );
      ...
      await agreement.setStorageUint256(
        ...
        BigNumber.from(NEXT_TWO_MONTH + 7 * ONE_DAY)
      );
      await agreement.setStorageUint256(hex4Bytes('P1'), BigNumber.from(MAX_PERCENT));
      ...
      if (!GP_FAILS_TO_DO_GAP_DEPOSIT) {
        ...
        await agreement.setStorageUint256(hex4Bytes('WHALE'), BigNumber.from(WHALE));
